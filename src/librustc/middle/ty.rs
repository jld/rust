...
#[deriving(Eq, IterBytes)] #[repr(uint)] // NOTE can reduce size to u8 after snapshot
...
// META-FIXME: make this an iterator, not a for-thing
pub fn each_attr(tcx: ctxt, did: def_id, f: &fn(@ast::meta_item) -> bool) -> bool {
    if is_local(did) {
        ...
        do csearch::get_item_attrs(tcx.cstore, did) |meta_items| {
            ret = ret && meta_items.each(|ptrptr| f(*ptrptr));
        }
        ret
    }
}
